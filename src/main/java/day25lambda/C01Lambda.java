...
public class C01Lambda {
    public static void main(String[] args) {
        ...
        System.out.println(getSquaresOfLastDigitsOfEvens(12, 16));
    ...
    public static int getSquaresOfLastDigitsOfEvens(int s, int e){

        //return IntStream.rangeClosed(s,e).filter(t-> t%2==0).map(t-> t%10).map(t-> t*t).sum();
        return IntStream.rangeClosed(s,e).filter(Utils::isEven).map(t-> t%10).map(Utils::getSquare).sum();
